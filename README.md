...
Toda la tarjeta se puede pulsar para desplegar el botón "¿Hablamos?" (con animación, sin recargar ni navegar) — vive solo ahí, no en la tarjeta cerrada: entrar al detalle es lo que habilita poder escribir, no algo que esté siempre a la vista. Si has escrito `details`, se despliega justo debajo de `description` a la vez que el botón — la corta se ve siempre, la larga solo al desplegar. "¿Hablamos?" abre el modal de contacto ampliado (el mismo correo/teléfono del masthead, en grande; su clic no vuelve a plegar la tarjeta) — quien esté interesado escribe y ahí se habla el alcance y el precio si no lo has puesto ya en `priceRange`.
